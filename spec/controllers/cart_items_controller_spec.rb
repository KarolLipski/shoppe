...
RSpec.describe CartItemsController, type: :controller do
  ...
  describe 'POST create' do
    ...
    it 'assigns current cart' do
      xhr :post, :create, stored_item_id: @stored_item.id, quantity: 2
      expect(assigns(:cart).user_id).to eq @user.id
    end
    context 'when data is valid' do
      it 'creates new cart_item' do
        xhr :post, :create, stored_item_id: @stored_item.id, quantity: 2
        expect(assigns(:cart_item).stored_item_id).to eq @stored_item.id
      end
    ...
    context 'when data is invalid' do
      it 'doesnt create cart_item' do
        xhr :post, :create, stored_item_id: @stored_item.id, quantity: 'abc'
      ...
      it 'renders save template' do
        xhr :post, :create, item_id: @stored_item.id, quantity: 'abc'
        expect(response).to render_template('save')
      end
    end
    context 'when user is not logged' do
      it 'sets error message' do
        log_out
        xhr :post, :create, stored_item_id: @stored_item.id, quantity: '23'
        expect(assigns(:errors).size).to eq 1
      end
    end
  end
  ...
  describe 'POST Update' do
    before(:each) do
      ...
      @cart = current_cart
      @stored_item = FactoryGirl.create(:stored_item)
      @cart_item = FactoryGirl.create(:cart_item, stored_item: @stored_item, cart: @cart)
    end
    context 'when data is valid' do
